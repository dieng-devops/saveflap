...
root.createDatatableWithContextMenu = (element, options, search_fields) ->
  menu_opts =
    ajax:
      url: options['source']
      type: 'POST'
  ...
  table = createDatatableWithButtons(element, new_options, search_fields)
...
root.createDatatableWithButtons = (element, options, search_fields) ->
  # Get all buttons from element
  all_buttons = options['buttons']
  ...
  table = createDatatable(element, new_options, search_fields)
  return table


root.createDatatable = (element, options, search_fields) ->
  table = $(element).DataTable(options)
  filters = options['filters']
  init_yadcf(element, table, search_fields, filters)
  return table
...
init_yadcf = (element, datatable, search_fields, filters) ->
  ...
  if filters?
    yadcf.exFilterColumn(datatable, filters)
